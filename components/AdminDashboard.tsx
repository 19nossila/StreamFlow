...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ onLogout, onPreview }) => {
  ...
  const [driveStatus, setDriveStatus] = useState<'disconnected' | 'connected'>('disconnected');
  ...
  const handleConnectDrive = async () => {
      ...
      try {
          ...
          setSuccessMsg("Connected to Google Drive.");
      } catch (e: any) {
  ...
  const handlePushToCloud = async () => {
      ...
      try {
          const data = storageService.exportData();
          await googleDriveService.uploadData(data);
          setSuccessMsg("Data successfully saved to Google Drive (streamflow_backup.json)");
      } catch (e: any) {
          setError("Upload failed: " + (e.result?.error?.message || e.message));
      } finally {
          setDriveLoading(false);
      }
  };
  ...
  const handlePullFromCloud = async () => {
      ...
      try {
          ...
          setError("Download failed: " + (e.result?.error?.message || e.message));
      } finally {
          setDriveLoading(false);
      }
  };
        ...
        <main className="flex-1 p-6 overflow-y-auto">
          ...
          {activeTab === 'users' && (
            <div className="max-w-4xl mx-auto">
              {/* User Content Omitted for brevity, same as previous */}
          ...
          {activeTab === 'settings' && (
              <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                  ...
                  <div className="bg-gray-800 rounded-lg p-6 border border-gray-700 shadow-xl">
                      <div className="text-center mb-4">
                          <div className="w-12 h-12 bg-gray-700 rounded-full flex items-center justify-center mx-auto mb-2">
                              <i className="fas fa-key text-xl text-red-500"></i>
                          </div>
                      ...
                      <form onSubmit={handleUpdatePassword} className="space-y-4">
                          <div>
                              ...
                              <input required type="password" value={passwords.new} onChange={e => setPasswords({ ...passwords, new: e.target.value })} className="w-full bg-gray-900 border border-gray-600 rounded p-2 text-white" />
                          </div>
                          <div>
                              <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Confirm</label>
                              <input required type="password" value={passwords.confirm} onChange={e => setPasswords({ ...passwords, confirm: e.target.value })} className="w-full bg-gray-900 border border-gray-600 rounded p-2 text-white" />
                          </div>
                          <button type="submit" className="w-full bg-red-600 hover:bg-red-700 py-2 rounded text-white font-bold text-sm">Update</button>
                      </form>
                  </div>
                  ...
                  <div className="bg-gray-800 rounded-lg p-6 border border-gray-700 shadow-xl">
                      <div className="text-center mb-4">
                          <div className="w-12 h-12 bg-gray-700 rounded-full flex items-center justify-center mx-auto mb-2">
                              <i className="fas fa-file-export text-xl text-blue-500"></i>
                          </div>
                          <h3 className="font-bold text-white">Manual File Backup</h3>
                      </div>

                      <div className="space-y-4">
                          <button onClick={handleExportBackup} className="w-full bg-blue-600 hover:bg-blue-700 py-2 rounded text-white font-bold text-sm flex items-center justify-center gap-2">
                              <i className="fas fa-download"></i> Export JSON
                          </button>

                          <div className="relative">
                              <button className="w-full bg-gray-700 hover:bg-gray-600 py-2 rounded text-white font-bold text-sm flex items-center justify-center gap-2 pointer-events-none">
                  ...
                  <div className={`bg-gray-800 rounded-lg p-6 border ${driveStatus === 'connected' ? 'border-green-500/50' : 'border-gray-700'} shadow-xl relative overflow-hidden`}>
                      {!isDriveReady ? (
                          <div className="absolute inset-0 bg-gray-900/90 flex flex-col items-center justify-center p-4 text-center z-10">
                              <i className="fas fa-exclamation-triangle text-yellow-500 text-3xl mb-2"></i>
                              <p className="font-bold">Not Configured</p>
                              <p className="text-xs text-gray-400 mt-1">Add Google Client ID in <code>services/googleDrive.ts</code></p>
                          </div>
                      ) : null}

                      <div className="text-center mb-4">
                          <div className={`w-12 h-12 rounded-full flex items-center justify-center mx-auto mb-2 ${driveStatus === 'connected' ? 'bg-green-900/30' : 'bg-gray-700'}`}>
                              <i className="fab fa-google-drive text-xl text-green-400"></i>
                          </div>
                          <h3 className="font-bold text-white">Google Drive Sync</h3>
                          <p className="text-xs text-gray-400">{driveStatus === 'connected' ? 'Connected' : 'Disconnected'}</p>
                      </div>

                      {driveStatus === 'disconnected' ? (
                          <button 
                             onClick={handleConnectDrive} 
                             disabled={!isDriveReady || driveLoading}
                             className="w-full bg-white hover:bg-gray-200 text-gray-900 py-2 rounded font-bold text-sm flex items-center justify-center gap-2 disabled:opacity-50"
                          >
                              {driveLoading ? 'Connecting...' : <><i className="fab fa-google"></i> Connect</>}
                          </button>
                      ) : (
                          <div className="space-y-3">
                              <button 
                                ...
                                className="w-full bg-green-600 hover:bg-green-700 py-2 rounded text-white font-bold text-sm flex items-center justify-center gap-2 disabled:opacity-50"
                              >
                                  {driveLoading ? 'Uploading...' : <><i className="fas fa-cloud-upload-alt"></i> Push to Cloud</>}
                              </button>
                              <button 
                                onClick={handlePullFromCloud} 
                                disabled={driveLoading}
                                className="w-full bg-yellow-600 hover:bg-yellow-700 py-2 rounded text-white font-bold text-sm flex items-center justify-center gap-2 disabled:opacity-50"
                              >
                                  {driveLoading ? 'Downloading...' : <><i className="fas fa-cloud-download-alt"></i> Pull from Cloud</>}
                              </button>
                          </div>
                      )}
                  </div>
